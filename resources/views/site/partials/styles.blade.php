	
	
	{{-- <link rel="shortcut icon" href="{{ asset('frontend/images/favicon.ico') }}" type="image/x-icon"> --}}
	{{-- <link rel="apple-touch-icon" type="image/x-icon" href="{{ asset('frontend/images/apple-touch-icon-57x57-precomposed.png') }}">
	<link rel="apple-touch-icon" type="image/x-icon" sizes="72x72" href="{{ asset('frontend/images/apple-touch-icon-72x72-precomposed.png') }}">
	<link rel="apple-touch-icon" type="image/x-icon" sizes="114x114" href="{{ asset('frontend/images/apple-touch-icon-114x114-precomposed.png') }}">
	<link rel="apple-touch-icon" type="image/x-icon" sizes="144x144" href="{{ asset('frontend/images/apple-touch-icon-144x144-precomposed.png') }}"> --}}
	<!-- GOOGLE WEB FONT -->
	<link href="https://fonts.googleapis.com/css?family=Poppins:300,400,500,600,700|Satisfy" rel="stylesheet">
	{{-- Animate css --}}
	<link href="{{ asset('frontend/css/animate.min.css') }}" rel="stylesheet" type="text/css"/>
	{{-- Bootstrap 3 css --}}
	<link href="{{ asset('frontend/css/bootstrap.min.css') }}" rel="stylesheet" type="text/css"/>
	{{-- sit style css --}}
	<link href="{{ asset('frontend/css/style.css') }}" rel="stylesheet" type="text/css"/>
	{{-- responsive ui css --}}
	<link href="{{ asset('frontend/css/responsive-ui.css') }}" rel="stylesheet" type="text/css"/>
	{{-- menu css --}}
	<link href="{{ asset('frontend/css/menu.css') }}" rel="stylesheet" type="text/css"/>
	{{-- icon css --}}
	<link href="{{ asset('frontend/css/icon_fonts/css/all_icons.min.css') }}" rel="stylesheet" type="text/css"/>
	{{-- custom css --}}
	<link href="{{ asset('frontend/css/custom.css') }}" rel="stylesheet" type="text/css"/>
	<style>
		#logo_home h1 a {
		background-image: url({{url('/storage//'.config('settings.site_logo'))}});
		}
	</style>
	{{-- Modernizr js --}}
	<script src="{{ asset('frontend/js/modernizr.js') }}" type="text/javascript"></script>
		<!--[if lt IE 9]>
      <script src="{{asset('frontend/js/html5shiv.min.js')}}"></script>
      <script src="{{asset('frontend/js/respond.min.js')}}"></script>
    <![endif]-->

	{{-- <!-- jQuery -->
	<script src="{{ asset('frontend/js/jquery-3.4.0.min.js') }}" type="text/javascript"></script>

	<!-- Bootstrap 4 files-->
	<script src="{{ asset('frontend/bootstrap-4.5.3/js/bootstrap.bundle.min.js') }}" type="text/javascript"></script>
	<link href="{{ asset('frontend/bootstrap-4.5.3/css/bootstrap.css') }}" rel="stylesheet" type="text/css"/>

	<!-- Font awesome 5 -->
	<link href="{{ asset('frontend/fonts/fontawesome/css/all.min.css') }} " type="text/css" rel="stylesheet">
	<link rel="stylesheet" href="{{asset('frontend/plugins/fancybox/fancybox.min.css')}}" type="text/css" media="screen" />
	<link rel="stylesheet" href="{{asset('frontend/plugins/bootstrapslider/bootstrap-slider.min.css')}}" type="text/css" media="screen" />


	<!-- custom style -->
	<link href="{{ asset('frontend/css/ui.css') }}" rel="stylesheet" type="text/css"/>
	<link href="{{ asset('frontend/css/responsive.css') }}" rel="stylesheet" media="only screen and (max-width: 1200px)" />

	<!-- custom javascript -->
	<script src="{{ asset('frontend/plugins/bootstrapslider/bootstrap-slider.min.js') }}" type="text/javascript"></script>
	<script src="{{ asset('frontend/js/script.js') }}" type="text/javascript"></script> --}}
